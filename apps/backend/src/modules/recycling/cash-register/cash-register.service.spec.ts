...
describe('CashRegisterService', () => {
  ...
  describe('close', () => {
    it('should throw BadRequestException when no open session', async () => {
      ...
      await expect(service.close(TENANT, OPERATOR)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('getTransactions', () => {
    it('should throw NotFoundException when session not found', async () => {
      mockSessionRepo.findOne.mockResolvedValue(null);
      await expect(
        service.getTransactions(TENANT, 'non-existent-id'),
    ...
    it('should return transactions for a valid session', async () => {
      ...
      mockSessionRepo.findOne.mockResolvedValue(session);
      const txs = [
        { id: 't1', cashSessionId: 's1', type: TransactionType.IN, amount: 50 },
      ];
